...
export const selectRecurringStatus = (state: { recurring: RecurringState }) => state.recurring.status;
...
export const selectPausedRecurring = createSelector(
  [selectAllRecurring],
  (items) => items.filter(r => !r.isActive)
);

export const selectRecurringById = (id: string) => createSelector(
  [selectAllRecurring],
...
export const selectDueRecurring = createSelector(
  [selectActiveRecurring],
  (activeItems) => {
    const today = new Date().toISOString().split('T')[0];
    return activeItems.filter(r => r.nextExecution <= today);
  }
);

export const selectMonthlyRecurringTotal = createSelector(
  [selectActiveRecurring],
  (active): { expenses: number; income: number } => {
